...
/** 화면에 필요한 것만 추린다. 저장 시각과 출처 공간 ID는 보내지 않는다. */
    ...
    async GET(request: Request) {
      try {
        ...
        return json({
          today: day,
          saved: cards.length,
          due: dueToday(cards, day, DAILY_REVIEW_SIZE).length,
          cards: all
            ? cards.sort((a, b) => a.due_on.localeCompare(b.due_on) || a.id.localeCompare(b.id))
            : dueToday(cards, day, DAILY_REVIEW_SIZE),
        });
      } catch (error) {
