...
export function SpeechPanel({ workspaceId, ownerId, stopped, stopToken, stopSpeechRef, onMedia, stopVideo, writingLink = true }: {
  ...
  async function record() {
    await stop(); await stopVideo(); const token = generation.current;
    if (typeof MediaRecorder === 'undefined' || !navigator.mediaDevices?.getUserMedia) throw new Error('이 브라우저는 녹음을 지원하지 않아요.');
    setSaved('마이크 권한 확인 중…');
    const media = await navigator.mediaDevices.getUserMedia({ audio: true }).catch(() => { throw new Error('마이크를 허용하지 않았거나 사용할 수 없어요. 브라우저의 마이크 권한을 확인해 주세요.'); });
  ...
  return <section id="learning-speech" className="space-y-4 rounded-xl border border-border bg-card p-4 sm:p-5" aria-label="말하기 연습">
    ...
    {!enabled && <p role="status" className="text-sm">지금은 AI를 사용할 수 없어요. 녹음은 이 기기에 계속 보관할 수 있어요.</p>}
...
export function SpeechResult({ item, busy, readOnly = false, onCommand, onPlay }: { item: LearningSpeech; busy: boolean; readOnly?: boolean; onCommand: (action: string, extra?: Record<string, unknown>) => Promise<void>; onPlay: () => Promise<void> }) {
