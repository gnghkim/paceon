...
export function ExpressionReview() {
  ...
  // 한 번에 세 개씩 묻는다. 다 하고 나서 더 남아 있으면 그때 물어본다.
  const [more, setMore] = useState(0);
  useEffect(() => {
    if (!data || index < data.cards.length) return;
    const controller = new AbortController();
    void apiFetch('/api/learning/expressions', { signal: controller.signal, cache: 'no-store' })
      .then((response) => (response.ok ? response.json() : null))
      .then((body: Payload | null) => {
        if (!controller.signal.aborted && body) setMore(body.due);
      })
      .catch(() => { /* 더 있는지는 보조 정보다. */ });
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <header>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          저장한 표현을 간격을 두고 다시 꺼내 봐요. 하루 {DAILY_REVIEW_SIZE}개면 충분해요.
        </p>
      </header>
      ...
      {data && data.saved > 0 && (
        <p className="text-sm text-muted-foreground">복습함에 {data.saved}개 저장돼 있어요.</p>
      )}
    </div>
  );
}
