...
저장한 영상 목록에서 아직 이름을 붙이지 않은 영상은 YouTube 공개 oEmbed(`https://www.youtube.com/oembed`)로 제목만 받아 화면에 표시합니다. 계정 연결이나 API 키가 필요 없으며, 썸네일과 같이 `no-store`로 화면에만 쓰고 저장하지 않습니다. 요청이 실패하면 저장된 이름을 그대로 보여줍니다.
