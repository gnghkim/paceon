...
PC 사이드바와 모바일 하단은 **오늘 / 영어 학습 / 캘린더 / 서재 / 통계** 순서다. **독서 기록**은 두 기기 모두 상단의 별도 버튼으로 연다. 설정은 상단 아이콘과 PC 사이드바 하단에서 열며, 모바일 로그아웃은 설정의 **계정**에 있다.

서재는 **전체 / 책 / 교재·강의**로 구분한다. 책의 **전체 / 읽는 중 / 완독 / 보관** 필터는 책을 선택했을 때만 표시한다. 제목·저자 검색은 현재 선택한 종류에 적용한다. **자료 추가**(`/resources/add`)에서 책 검색·직접 입력·PDF 가져오기 또는 교재·강의 등록을 고른다. 기존 개별 등록 주소도 유지한다.

설정의 **요일별 학습 가능 시간**은 책이나 계획이 없어도 입력할 수 있다. 강의 계획에서 설정으로 이동한 경우 저장 뒤 **계획으로 돌아가기**로 원래 자료에 복귀한다. 캘린더의 책 일정은 페이지 범위와 책 상세로, 교재·강의 일정은 챕터 정보와 해당 자료 상세로 연결한다.
