...
작성일: 2026-09-12  
상태: Phase 8 Learning Statistics 구현 및 검증 완료 (최신 기록은 §20)
...
## 17. Phase 5 실행 기록 — 2026-09-13

Phase 4 커밋 `b284fe5`에서 `feat/phase-5-progress-replanning` 브랜치를 만들었다.

- 실제 읽기, 복습, 마지막 읽기 정정/무효 처리와 유효 진도 projection을 연결했다. 날짜·시간·메모와 변경 전후 예상 완료일, 정정 이력을 표시한다.
- 최근 유효한 읽기 시간으로 속도를 계산하고 미래의 변경 가능한 세션을 재계획한다. 과거·오늘·고정·진행 중·완료·이력 연결 세션은 보존한다.
- 기록과 재계획을 사용자 잠금·버전 검증·멱등성 ledger가 있는 단일 RPC로 저장한다. 일정 충돌은 실제 기록을 보존하고 조정 대기 상태로 남긴다. 계획 방식·분량·목표일을 바꿔 다시 적용할 수 있다.
- 완독 후 정정으로 다시 읽기 상태로 돌아갈 수 있다. 이때 일정 충돌이 있더라도 자료와 계획의 상태를 함께 갱신한다. 이력 전체 되돌리기 대신 마지막 읽기 기록의 정정/무효 처리를 제공한다.
- Node 53개와 Scheduler 52개, pgTAP 93개가 통과했다. 실제 Auth/프로덕션 HTTP 통합 테스트에서 동시 중복 요청, 오래된 버전, 사용자 격리, 정정·복습, 완독·재개, 느린 실측 속도로 인한 충돌과 수동 복구를 검증했다.
- 타입 검사, lint, 프로덕션 빌드, DB lint와 생성 타입 일치 검사를 통과했다. 브라우저에서 신규 가입과 계획 저장, 미달·초과 기록, 복습·정정·전체 무효 처리, 새로고침 유지와 390px 모바일 화면을 확인했다.

화면과 API 계약은 [WORKSPACE_UI.md](WORKSPACE_UI.md)를 따른다. 다음은 **Phase 6 AI**이며 난이도·예상 시간·Coach의 범위를 정한 뒤 진행한다.

## 18. Phase 6 실행 기록 — 2026-09-13

Phase 5를 `ffc027c`로 커밋하고 `feat/phase-6-ai` 브랜치에서 구현했다.

- 도서 분석(난이도·전체 예상 시간·중요도·신뢰도·근거)과 자료별 기본 학습 코칭을 연결했다. 목차를 선택적으로 입력하며 개인 메모와 로그인 정보는 모델에 보내지 않는다.
- PostgreSQL 작업 큐와 RLS, 중복 억제, 5개 동시 작업 한도, Worker lease/재시도/CAS를 구현했다. Worker가 중단되어도 요청을 보존하며 오래된 Worker의 결과 덮어쓰기를 차단한다.
- OpenAI Responses strict JSON Schema 요청, Python Pydantic·SQL·웹 Zod 검증, 안전한 오류 상태와 모델/토큰 사용 이력을 구현했다. 제공자 키와 모델이 없으면 소비자를 시작하지 않는다.
- AI는 참고용 분석만 저장한다. 날짜·진도·완료 예상일·일정은 기존 결정론적 경로에서 계산하며 AI 작업 전후 계획과 일정이 동일함을 통합 테스트로 확인했다.
- 캐시된 목차 A → B → A 재요청이 마지막 선택 A를 표시하도록 수정하고 회귀 테스트를 추가했다.
- Node 60개, Scheduler 52개, Python Worker 14개, pgTAP 132개를 통과했다. 실제 프로덕션 Next/Auth/DB와 Python Worker를 연결하고 외부 제공자만 fixture로 대체한 통합 테스트를 통과했다. 타입 검사·lint·웹/Worker 빌드·생존 확인·DB lint·생성 타입 일치도 확인했다.
- 브라우저에서 미설정 안내, 분석·코칭 접수와 완료, 실패 후 재시도, 실제 10쪽 학습 기록 뒤 이전 정보 표시를 확인했다. 390px 모바일에서 가로 넘침과 브라우저 실행 오류가 없었다. 검증 결과는 테스트 전용 fixture이며 실모델 출력으로 표시하지 않는다.

실제 AI 키가 없어 유료 모델 실호출과 분석 품질은 아직 검증하지 않았다. 로컬 AI는 기본 비활성 상태이며 [AI.md](AI.md)의 설정 후 사용할 수 있다. PDF 파싱·학습단위 분석은 후속 범위다.

## 19. Phase 7 실행 기록 — 2026-09-13

Phase 6을 `b727211`로 커밋하고 `feat/phase-7-pdf-import`에서 구현했다.

- 비공개 PDF 업로드, 영속 작업 큐, 페이지·최상위 책갈피·본문 일부 추출, 결과 확인 후 교재 등록을 연결했다. 기존 페이지 기반 계획·진도·재계획과 호환되는 `BOOK/PAGE` 자료와 단원을 원자적으로 저장한다.
- 최대 10MiB·500페이지, 소유자별 Storage/RLS, 파일 해시를 결합한 중복 요청 처리, Worker lease와 결과 CAS를 적용했다. 원본은 인증된 다운로드로 제공하며 삭제 실패 시 같은 객체의 정리를 재시도할 수 있다.
- pypdf 파서는 인증 정보 없는 별도 프로세스에서 시간·메모리·출력 제한을 적용한다. 암호화·손상 파일을 구분하고, 스캔 PDF는 OCR 없이 페이지 수로 등록한다. NUL·잘못된 Unicode와 UTF-16 길이 경계도 검증했다.
- 추출한 목차와 본문 발췌를 AI 입력의 초기값으로 제공하며 사용자 수정·비우기를 유지한다. PDF 처리는 OpenAI 키 없이 동작한다. 로컬 PDF Worker를 활성화했고 AI는 비활성 상태를 유지한다.
- Node 68개와 Scheduler 52개, Python Worker 25개, pgTAP 197개가 통과했다. 실제 프로덕션 Next/Auth/Storage/Python 파서 통합 테스트에서 업로드·소유권 격리·등록 멱등성·원본 다운로드·계획·진도·실패 재시도를 확인했다. 임시 객체를 Storage API로 제거한 후 임시 계정을 정리했다.
- frozen lockfile 설치, 타입 검사, lint, 웹/Worker 빌드, DB lint와 생성 타입 일치 검사를 통과했다. 통합 테스트 도구의 동기 Docker 실행을 비동기로 바꾸고 추가 진단을 적용한 뒤 다시 통과했다.
- 브라우저에서 실제 업로드 → 자동 추출(6쪽/3단원) → 제목·현재 페이지 확인 → 서재 등록 → 계획 저장 → 2–3쪽 학습 기록(진도 3/6쪽)을 확인했다. 업로드·등록 버튼에 누락된 submit 타입을 수정해 재검증했다. 데스크톱·390px 모바일 화면에 가로 넘침이 없었다.
- 원본 다운로드 API의 인증·바이트 일치는 통합 테스트로 검증했다. 브라우저 다운로드 이벤트는 발생했지만 자동화 브라우저가 저장을 취소해 실제 파일 저장 완료는 확인하지 못했다. 저장 경로 설정과 객체 URL 해제 중단으로도 같았으며 제품 코드는 변경하지 않았다.

처리 범위와 실행 방법은 [PDF.md](PDF.md)를 따른다. OCR, 중첩 단원 편집과 비밀번호 해제는 후속 범위다.

## 20. Phase 8 실행 기록 — 2026-09-13

Phase 7을 `1c73527`로 커밋하고 `feat/phase-8-statistics`에서 구현했다. 기존 단계별 계획 이후의 확장 범위로 PRD P1의 Statistics를 선택했다.

- `/statistics`와 인증된 `/api/statistics`를 연결했다. 최근 7·30·90일, 최대 366일 사용자 지정 기간, 신규 학습·복습 분량, 기록 시간·미입력 건수, 활동일과 날짜별·자료별 집계를 제공한다.
- 전체 이력에서 무효 관계를 해소한 뒤 저장된 학습일로 기간을 선택한다. 등록 당시 진도는 기간 실적에 포함하지 않으며 정정된 기록은 대체 학습일로 반영한다. 표본 없는 속도와 시간 미입력을 추측하지 않는다.
- 기존 사용자 토큰/RLS와 페이지네이션을 재사용한다. 테이블당 조회 한도에 도달하거나 학습·복습 페이지 범위가 잘못되면 불완전한 합계를 표시하지 않는다. 통계는 메모를 반환하거나 계획·진도·일정을 수정하지 않는다.
- Node 79개와 Scheduler 52개를 통과했다. 실제 Auth·프로덕션 웹·DB 통합 테스트에서 학습·복습·정정 집계, 다른 사용자의 자료 격리, 조회 전후 업무 상태 불변을 검증했다. 독립 리뷰의 잘못된 복습 분량 문제를 실패 테스트로 재현하고 수정했다.
- 타입 검사·lint·프로덕션 빌드를 통과했다. 브라우저에서 빈 상태, 7·90일 선택, 366일 초과 안내, 네트워크 오류 후 재시도와 실제 10쪽·5분 기록의 집계(활동 1일, 0.5분/쪽)를 확인했다. 390px 모바일·데스크톱 화면과 실행 오류를 확인하고 임시 계정·자료를 정리했다. 기존 관리자 계정은 유지했다.

지표와 실행 방법은 [STATISTICS.md](STATISTICS.md)를 따른다. 대용량 DB 집계, 계획 수행률과 이해도 추정은 이번 단계에 포함하지 않는다.
