...
## 16. Phase 4 실행 기록 — 2026-09-13

Phase 3 커밋 `0bdf8a8`에서 `feat/phase-4-basic-ui` 브랜치를 만들었다.

- 이메일 로그인·가입·로그아웃과 세션 갱신, 인증된 화면 경계, 반응형 사이드바/하단 내비게이션을 연결했다. 계정 전환 시 개인 폼 상태를 초기화한다.
- Today, 제목·저자 검색/상태 필터가 있는 서재, 도서 상세, Google/수동 등록, 일·주·월 캘린더를 구현했다. 예상 완독과 등록 시 진도를 저장된 값으로 표시한다.
- 세 가지 계획 모드, 페이지당 예상 시간, 사용자 시간대, 요일·시간 예산을 입력해 미리보기 후 최초 계획을 저장한다. 기존 책의 시간 예약을 반영하며 공통 설정을 묵시적으로 바꾸지 않는다.
- RLS를 유지하는 최초 계획 RPC를 추가했다. 전체 페이지·최초 진도 snapshot, 기존 계획/이력, 페이지 전체 연속성과 시간 용량을 트랜잭션 안에서 검증한다. 목표·계획·세션을 함께 저장하며 중복/동시 저장 실패는 전체 취소한다.
- Node 테스트 33개 + Scheduler Vitest 52개, pgTAP 75개, 실제 프로덕션 Next/Auth/RLS/RPC HTTP 통합 테스트가 통과했다. 서로 다른 책의 동시 시간 경쟁, 중복 저장과 실패 시 불필요한 목표 행이 없는지 확인했다.
- 리뷰에서 프로필 조회 정렬 컬럼 오류, 50쪽 × 1.1분의 부동소수점 올림, 계정 전환 시 폼 상태 잔류를 발견하고 수정했다. 소수 시간은 정수 밀리분/PostgreSQL numeric 기준으로 맞췄다.
- 브라우저 가입 → Google 검색 실패 안내 → 수동 등록(320쪽/현재80쪽) → 계획 미리보기/저장 → 새로고침 → Today·Calendar → 서재 검색 → 로그아웃/재로그인을 확인했다. 1440px 데스크톱과 390px 모바일에서 가로 넘침·브라우저 실행 오류가 없었다. 임시 계정과 자료는 삭제했다.
- frozen lockfile 설치, 타입 검사, lint, 빌드, DB lint와 생성 타입 일치 검사를 통과했다. 로컬 웹 환경을 연결했으며 실제 환경 파일은 Git에서 제외했다.

사용법과 API는 [WORKSPACE_UI.md](WORKSPACE_UI.md)에 기록했다. 다음은 **Phase 5 Progress / Replanning**이다. 실제 학습량 기록, 유효 진도 projection, 자동 재계획의 원자적 저장을 구현한다. 현재 화면은 최초 계획과 등록 당시의 진도를 제공한다.
