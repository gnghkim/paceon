...
초기 구현은 키 없는 고정 응답 테스트로 검증했다. 2026-09-13에는 실제 로컬 키를 연결하고 `gpt-5.4-mini`로 YES24 검색 → 임시 교재 등록 → 도서 분석·코칭 각 1건의 완료와 구조화 결과 저장을 확인했다. 모델 선택은 [OpenAI 공식 문서](https://developers.openai.com/api/docs/models/gpt-5.4-mini)의 Responses·구조화 출력 지원과 비용·속도를 기준으로 했다. 실제 호출 성공은 확인했지만 다양한 도서에 대한 분석 품질 평가는 별도로 필요하다. 테스트 응답은 테스트 전용 도구에만 있으며 운영 Worker에 가짜 결과 모드가 없다.

YES24와 실제 AI를 함께 확인하려면 먼저 위 Worker/웹 설정과 `apps/web/.env.local`의 `YES24_API_KEY`를 설정한다. `pnpm test:providers:live`는 실행 중인 localhost:3000과 로컬 Worker를 통해 실제 YES24 검색 → 임시 교재 등록 → 도서 분석·코칭 각 1건을 요청한다. **실제 OpenAI 사용료가 발생할 수 있으므로 일반 테스트에는 포함하지 않는다.** 키가 없으면 호출 전에 중단한다. 검증 후 임시 계정·자료를 삭제하며 기존 계정은 변경하지 않는다. 기존 로컬 PDF 설정은 유지한다. 실제 연결 완료 여부는 이 명령의 결과로 확인하며 단위 테스트 통과로 대체하지 않는다.

브라우저에서는 미설정 안내와 분석/코칭 대기·완료·실패·재시도를 확인했다. 실제 읽기 기록을 저장한 뒤 기존 AI 결과가 이전 정보로 표시되는 것을 확인했고, 390px 모바일에서 가로 넘침과 실행 오류가 없었다.

Phase 7에서 PDF 업로드·파싱과 책갈피 단원을 연결했다. PDF에서 추출한 목차·본문 발췌는 AI 카드에 제안되며 사용자가 요청할 때만 분석에 보낸다. [PDF.md](PDF.md)를 참고한다. 세부 AI 학습단위 분석, mastery 평가, 자유 대화 Coach는 후속 범위다. 현재 Coach는 자료별 짧은 설명 카드다.
